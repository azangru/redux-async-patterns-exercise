...
import {showcaseFetcher, tabFetcher, resourcesFetcher} from '~/api/showcase';
...
function* loadCardsForResources({payload: resourcesData}) {
    try {
        const resourceUrls = resourcesData.map((resourceData) => resourceData.url);
        const fetchedResources = yield call(resourcesFetcher, resourceUrls);
